...
def decode_token_any(token: str) -> dict:
    """Decode a bearer token, newest scheme first: a Phase 2 SSO RS256 access
    token (verified via the issuer's JWKS and required to carry
    ``aud == this service``), else a legacy HS256 token signed with this
    service's own secret. Raises jose errors if neither validates."""
