...
def attach_provenance(user, ttl_data):
    ...
    # end_time = (datetime.datetime.utcnow() + datetime.timedelta(seconds=1)).isoformat() + "Z"  # End time slightly after start, not using for the time being.

    # Generate a unique UUID for the provenance entity
    provenance_uuid = str(uuid.uuid4())
    prov_entity = URIRef(BASE[f"provenance/{provenance_uuid}"])

    # Define a specific activity name "IngestionActivity" to define that this is a ingestion task with a UUID.
    ingestion_activity = URIRef(BASE[f"ingestionActivity/{provenance_uuid}"])


    user_uri = URIRef(BASE[f"agent/{user}"])

    # Define the provenance entity
    ...
    prov_graph.add((prov_entity, PROV.wasAttributedTo, user_uri)) #saying this provenance was AttributedTo some user
    prov_graph.add((prov_entity, PROV.wasGeneratedBy, ingestion_activity)) #this provenance information was generated by ingestion_activity

    # Define the ingestion activity
    ...
    # Link provenance to original ttl data
    for entity in original_graph.subjects():
        if isinstance(entity, URIRef):  # Ensure only URI-based subjects are linked
            ...
            prov_graph.add((ingestion_activity, PROV.wasAssociatedWith, entity))  # Link ingestion activity

    # Attach a Dublin Core provenance statement -- this is the new addition to say it's ingested by user
